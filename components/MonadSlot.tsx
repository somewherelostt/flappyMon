...
export const MonadSlot: React.FC<MonadSlotProps> = ({
  ...
  return (
    <div
      ref={slotRef}
      className={`Monad-slot ${className} ${clickable ? 'cursor-pointer hover:bg-secondary transition-colors' : ''}`}
      onClick={handleSlotClick}
      style={{
        width: dimensions.width,
        height: dimensions.height,
...
export class USDCService {
  ...
  static isSupportedChain(chainId: number): boolean {
    return chainId === 137 || chainId === 80002; // monad Mainnet or Amoy
  }
